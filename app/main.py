...
alignment_service = AlignmentService(language=settings.alignment_language)
...
async def _handle_lyrics_task(payload: dict, update_progress: TaskProgressUpdater) -> dict:
    if not settings.minimax_api_key or settings.minimax_api_key == "your_api_key_here":
        raise ValueError("请先在程序目录下的 .env 文件中配置 MINIMAX_API_KEY，然后重启程序。")
...
app = FastAPI(title="Wayne Sun Music Generator", lifespan=lifespan)
